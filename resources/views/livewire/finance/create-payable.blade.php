<form wire:submit="createPayable">
    ...
    <div class="mb-2 grid grid-cols-1 sm:grid-cols-3 items-center">
        <x-input-label for="debt_code" :value="__('Ke Rekening')" />
    ...
    <div class="mb-2 grid grid-cols-1 sm:grid-cols-3 items-center">
        <x-input-label for="cred_code" :value="__('Akun Hutang')" />
        <div class="sm:col-span-2">
            <select wire:model="cred_code"
                class="mt-1 block py-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                <option value="">Pilih Akun Hutang</option>
                @foreach ($payables as $payable) <option value="{{ $payable->id }}">{{ $payable->acc_name }}</option>
                @endforeach
            </select>
    ...
    <div class="mb-2 grid grid-cols-1 sm:grid-cols-3 items-center">
        <x-input-label for="contact_id" :value="__('Kontak')" />
        <div class="sm:col-span-2">
            <select wire:model="contact_id"
                class="mt-1 block py-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                <option value="">Pilih Kontak</option>
                @foreach ($contacts as $contact) <option value="{{ $contact->id }}">{{ $contact->name }}</option>
                @endforeach
            </select>
            <x-input-error class="mt-1 text-xs" :messages="$errors->get('contact_id')" />
        </div>
    </div>
